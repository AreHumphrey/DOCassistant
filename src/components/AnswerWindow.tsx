...
export default function AnswerWindow({uid, pid}: AnswerWindowProps) {
    ...
    return (
        <div className="flex max-w-full flex-col p-4 bg-gray-100 rounded shadow">
            <div className="flex flex-row gap-1">
                <button onClick={() => copyToClipboard()} disabled={loading}>Копировать</button>
                <button onClick={() => printPDF()} disabled={loading}>Печать</button>
                <button onClick={() => generatePDF()} disabled={loading}>PDF</button>
            </div>
            {loading ? (
                ...
                <div className="w-full p-4 bg-white border rounded shadow h-44 overflow-y-auto">
